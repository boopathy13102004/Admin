...
// 📂 Paths
const viewsPath = path.join(__dirname, "../src/temp");
...
const connectToMega = async () => {
  ...
  return new Promise((resolve, reject) => {
    storage = new Storage({
      email: "divansan05@gmail.com",
      password: "Divansan00876",
      timeout: 60000,
    });
...
app.post("/uploads", upload.single("file"), async (req, res) => {
  try {
    ...
    if (!megaFileUrl) {
      return res.status(500).json({ error: "Failed to upload file to MEGA" });
    }
...
app.post("/students", upload.fields([
  ...
  async (req, res) => {
    try {
      ...
      // Upload files to MEGA and get URLs
      const documents = {};
      for (const key of Object.keys(req.files)) {
        const file = req.files[key][0];
        const megaFileUrl = await uploadToMega(file.path, file.filename);
        if (megaFileUrl) {
          documents[key] = megaFileUrl;
        } else {
          console.error(`❌ Failed to upload ${key} to MEGA`);
        }
      }
    ...
    const studentsWithPhotos = students.map((student) => ({
      ...
      documents: {
        aadharCard: student.documents.aadharCard
          ? `/uploads/${student.documents.aadharCard}`
          : null,
        communityCertificate: student.documents.communityCertificate
          ? `/uploads/${student.documents.communityCertificate}`
          : null,
        tenthMarkSheet: student.documents.tenthMarkSheet
          ? `/uploads/${student.documents.tenthMarkSheet}`
          : null,
        eleventhMarkSheet: student.documents.eleventhMarkSheet
          ? `/uploads/${student.documents.eleventhMarkSheet}`
          : null,
        twelfthMarkSheet: student.documents.twelfthMarkSheet
          ? `/uploads/${student.documents.twelfthMarkSheet}`
          : null,
        transferCertificate: student.documents.transferCertificate
          ? `/uploads/${student.documents.transferCertificate}`
          : null,
        birthCertificate: student.documents.birthCertificate
          ? `/uploads/${student.documents.birthCertificate}`
          : null,
        photo: student.documents.photo
          ? `/uploads/${student.documents.photo}`
          : null,
      },
    }));
...
app.get('/student/:id', async (req, res) => {
  ...
  res.json({
      student,
      documentLinks: student.documents  // Contains Mega file URLs
  });
});
